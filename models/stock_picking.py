...
class StockPicking(models.Model):
    ...
    has_errors_address_dhl = fields.Boolean('Has the address errors according to DHL?', default=False,
                                            readonly=True)
    ...
    def get_dhl_attachment(self):
        ...
        if self.carrier_tracking_ref:
            for ref in self.carrier_tracking_ref.split(','):
                ...
                attachment_name = 'DHL Label-%s.%s' % (ref, label_format)
    ...
    def _write(self, vals):
        # If we are writing the state (computed) and we are a stock.picking which uses DHL...
